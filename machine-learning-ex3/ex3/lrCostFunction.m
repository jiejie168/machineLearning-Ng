...
z=X*theta;% the parameter z for hypothesis: m*1 vector  X:m*(n+1), theta=(n+1)*1
...
grad=(1/m)*X'*(g-y); % X:m*(n+1), g-y: (m)*1  grad: (n+1)*1
temp=theta;
temp(1)=0;
grad=grad+(lambda/m)*temp;   % X:m*(n+1), g-y: (m)*1
